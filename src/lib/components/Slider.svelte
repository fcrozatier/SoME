...
<div class="h-[400px] sm:h-auto py-10">
	...
	>
		...
		>
			<button
				type="button"
				class="rotate-90 sm:rotate-0 text-nowrap origin-left relative -left-1"
				on:click={() => (value = 1)}>{label1}</button
			>
			<button
				type="button"
				class="rotate-90 sm:rotate-0 text-nowrap origin-left sm:block relative -left-1"
				on:click={() => (value = 3)}>{label3}</button
			>
			<button
				type="button"
				class="rotate-90 sm:rotate-0 text-nowrap origin-left relative -right-2"
				on:click={() => (value = 5)}>{label5}</button
			>
			<button
				type="button"
				class="rotate-90 sm:rotate-0 text-nowrap origin-left sm:block relative -right-4"
				on:click={() => (value = 7)}>{label7}</button
			>
			<button
				type="button"
				class="rotate-90 sm:rotate-0 text-nowrap origin-left relatite -right-0"
				on:click={() => (value = 9)}>{label9}</button
			>
		</div>
		<div class="h-[500px]">
			<input
...
<p class="">
	Your estimation: <span class="font-semibold">{value ? label(value) : 'Please make a choice'}</span
	>
</p>
