...
export const load = async ({ locals }) => {
	...
	>[] = await db.execute(sql`
			...
			where entries.state in ${[
		ENTRY_STATE.ActionRequired,
		ENTRY_STATE.WaitingForReview,
	]}
...
export const actions: Actions = {
	...
	action_required: formgate(
		AdminFollowUpActionRequiredForm,
		async (data, { locals }) => {
			assertIsAdmin(locals);
